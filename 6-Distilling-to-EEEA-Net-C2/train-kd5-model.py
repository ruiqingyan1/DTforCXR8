...
"""
5、 EEEA-Net-C2 + AutoformerV2-base
"""
...
def main():
    ...
    TEACHER_CKPT_PATH = '../5-AutoformerV2-base/model5.pth'
    ...
    student_model.reset_classifier(num_classes=N_CLASSES, dropout=0.2)

    teacher_model = darmo.create_model("autoformerv2_base", num_classes=1000, pretrained=True)
    teacher_model.head = nn.Sequential(
        nn.Linear(teacher_model.head.in_features, N_CLASSES),
        nn.Sigmoid()
    )
    ...
    torch.save(student_model.state_dict(), 'modeld5.pth')
